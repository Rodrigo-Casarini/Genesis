...
function Optimize-Services {
    ...
    # preservada.
    #
    # OTIMIZAÇÃO: busca TODOS os serviços do WMI em uma única
    # chamada (em vez de uma consulta filtrada por nome a cada
    # vez) e usa cache em memória para nunca verificar o mesmo
    # serviço duas vezes na mesma execução.
    # ------------------------------------------------------------
    ...
    function Test-IsMicrosoftService {
        ...
        # Carrega todos os serviços do WMI de uma vez só (apenas na primeira chamada)
        ...
        try {
            ...
            # Reforço rápido primeiro (caminho), evita checar assinatura à toa
            ...
            if ($isInSystem32) {
                ...
                # Só verifica assinatura digital (mais lento) se não estiver em System32
            ...
            $result = $false  # Em caso de dúvida, NÃO mexe (lado seguro)
        }
