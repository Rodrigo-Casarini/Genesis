...
function Optimize-Services {
    ...
    # Verifica se um nome de serviço bate com algum item da lista de
    # proteção, suportando curingas (*) para nomes que variam por
    # versão/fabricante.
    ...
    # ------------------------------------------------------------
    # FILTRO DE FABRICANTE — a verdadeira rede de segurança.
    # Em vez de listar manualmente cada app de terceiros, verificamos
    # o fabricante real do executável por trás do serviço. Só mexemos
    # em serviços que pertencem à Microsoft/Windows. Qualquer coisa
    # de terceiros é automaticamente preservada.
    # ------------------------------------------------------------
    function Test-IsMicrosoftService {
        ...
        try {
            ...
            if (-not $wmiSvc -or -not $wmiSvc.PathName) { return $false }

            # Extrai o caminho do executável (remove aspas e argumentos, ex: -k netsvcs)
            $rawPath = $wmiSvc.PathName.Trim()
            ...
            # Verifica o fabricante digital do arquivo
            $sig = Get-AuthenticodeSignature -LiteralPath $exePath -ErrorAction SilentlyContinue
            $isMsSigned = $false
            if ($sig -and $sig.SignerCertificate) {
                $isMsSigned = $sig.SignerCertificate.Subject -like "*Microsoft*"
            }

            # Reforço: verifica se está dentro da pasta do sistema
            $isInSystem32 = $exePath -like "$env:WINDIR\System32\*" -or $exePath -like "$env:WINDIR\SysWOW64\*"

            return ($isMsSigned -or $isInSystem32)
        }
        catch {
            return $false  # Em caso de dúvida, NÃO mexe (lado seguro)
        }
    }

    Write-Log "--- Verificando serviços de terceiros instalados ---"
    $allServices = Get-Service
    $thirdPartyCount = 0
    foreach ($svc in $allServices) {
        if (-not (Test-IsMicrosoftService $svc.Name)) {
            $thirdPartyCount++
        }
    }
    Write-Log "$thirdPartyCount serviços de terceiros detectados e ignorados automaticamente (Dell, Razer, Steam, anticheats, etc.)."
